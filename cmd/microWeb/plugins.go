...
type ITemplatePlugin interface {
	//called once to initalize the plugin.
	Init()
...
func GetPluginByResourcePath(fsPath string) (string, error) {
	pluginList := make([]pluginBinding, len(globalSettings.plugins))
	copy(pluginList[:], globalSettings.plugins[:])

	lessFunction := func(i, j int) bool {
		iDist := StringMatchLength(path.Join(globalSettings.staticResourcePath, pluginList[i].Binding), fsPath)
		jDist := StringMatchLength(path.Join(globalSettings.staticResourcePath, pluginList[j].Binding), fsPath)
		return iDist > jDist
	}
	sort.Slice(pluginList[:], lessFunction)

	for _, plugin := range pluginList {
		if StringMatchLength(path.Join(globalSettings.staticResourcePath, plugin.Binding), fsPath) == len(path.Join(globalSettings.staticResourcePath, plugin.Binding)) {
